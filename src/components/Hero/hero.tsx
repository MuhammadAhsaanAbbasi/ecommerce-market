    ...
    return (
        <section className={"relative overflow-hidden flex justify-center sm:m-0"}>
            {data.map((footer: IFooter) => (
                <div key={footer._id}
                    className={"w-full h-[554px] bg-gray-50 bg-hero flex items-center sm:bg-cover sm:bg-revert sm:bg-no-repeat bg-cover bg-center"} style={{ backgroundImage: `url(${footer.HeroImage})` }}>
                    <div className='flex flex-col justify-center gap-4 md:p-5 sm:p-[10%,30px] p-[50px,30px]'>
                        <h1 className="md:text-7xl md:m-[32px,0] sm:text-5xl sm:m-[24px,0px] md:whitespace-normal w-[60%] text-4xl m-[8px,0px]">{footer.headerTitle}</h1>
